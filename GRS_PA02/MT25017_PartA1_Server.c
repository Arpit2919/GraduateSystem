...
Message* create_message(int total_size)
{
    ...
    memset(msg->field1,'A',fs); memset(msg->field2,'B',fs);
    memset(msg->field3,'C',fs); memset(msg->field4,'D',fs);
    memset(msg->field5,'E',fs); memset(msg->field6,'F',fs);
    memset(msg->field7,'G',fs); memset(msg->field8,'H',fs);
...
//SAFE SEND
ssize_t send_full(int socket, const char *buffer, size_t length)
{
    size_t total_sent = 0;

    while(total_sent < length && g_running)
    {
...
void* client_handler(void *arg)
{
    ...
    char *buffer = malloc(g_message_size);
    ...
    while(g_running)
    {
        memcpy(buffer + fs*0, msg->field1, fs);
        memcpy(buffer + fs*1, msg->field2, fs);
        memcpy(buffer + fs*2, msg->field3, fs);
        memcpy(buffer + fs*3, msg->field4, fs);
        memcpy(buffer + fs*4, msg->field5, fs);
        memcpy(buffer + fs*5, msg->field6, fs);
        memcpy(buffer + fs*6, msg->field7, fs);
        memcpy(buffer + fs*7, msg->field8, fs);

        ssize_t s = send_full(sock, buffer, g_message_size);
        if(s <= 0) break;
